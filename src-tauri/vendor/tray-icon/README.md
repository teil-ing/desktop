> **Vendored copy** — tray-icon 0.24.2 from crates.io with upstream
> [PR #365](https://github.com/tauri-apps/tray-icon/pull/365) applied
> (`src/platform_impl/macos/mod.rs`): macOS 27 swallows left-clicks when an
> NSMenu is permanently attached to the status item. Remove once Tauri 2.x
> depends on tray-icon >= 0.25.1. See `[patch.crates-io]` in `src-tauri/Cargo.toml`.
