...
impl VideoEncoder {
    ...
    pub fn new<P: AsRef<Path>>(
        ...
        // PATCHED (teil.ing): declaring an audio stream that never receives a
        // single sample yields an MP4 with a degenerate AAC track — players
        // shrug, strict transcoders (bunny Stream) hard-fail on it. When audio
        // is disabled, keep the profile video-only.
        if !is_audio_disabled {
        ...
        // PATCHED (teil.ing): no audio descriptor when audio is disabled.
